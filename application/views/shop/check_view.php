...
<?php if( isset($id_check) ) : ?>
    ...
    <tr><th style="width:25%;">บาร์โค้ด</th><th>รหัสสินค้า</th><th style="width:15%; text-align:center;">เวลา</th><th style="width:10%; text-align:right;">ลบ</th></tr>
    <tbody id="rs">
...
<tr id="rs_{{id}}"><td>{{ barcode }}</td><td>{{ product }}</td><td align="center">{{ timestamp }}</td><td align="right"><button type="button" class="btn btn-minier btn-danger" onclick="delete_check({{id}}, '{{barcode}}')">ลบ</button></td></tr>
</script>

<script id="history_template" type="text/x-handlebars-template"> 
{{#each this}}
{{#if no_content}}
<tr ><td colspan="4" align="center">----- ไม่พบประวัติ  -----</td></tr>
{{else}}
<tr id="rs_{{id}}"><td>{{ barcode }}</td><td>{{ product }}</td><td align="center">{{ timestamp }}</td><td align="right"><button type="button" class="btn btn-minier btn-danger" onclick="delete_check({{id}}, '{{barcode}}')">ลบ</button></td></tr>
{{/if}}
{{/each}}
...
<tr id="row_{{barcode}}"><td>{{ barcode }}</td><td>{{ product }}</td><td align="right" id="{{barcode}}">{{ qty }}</td></tr>
</script>
<script>
...
function view_history()
{
	...
	if(qty != "" && qty != 0)
	{
		...
		$.ajax({
			...
			type:"POST", cache:"false", data:{ "id_check" : id_check, "qty" : qty },
			success: function(rs)
			{
				load_out();
...
function do_checking()
{
	...
	$("#barcode").val("");
	if(id_check == ""){ swal("พบข้อผิดพลาด !!", "ไม่พบ ID ของการตรวจนับ ลองออกจากการตรวจนับแล้วกลับเข้ามาใหม่", "error"); return false; }
	if($("#pause").val() == 1){ window.location.href = "<?php echo $this->home; ?>/just_pause"; }
	if(barcode != "" && id_check != "")
	{
		...
		$.ajax({
			...
			type:"POST", cache:"false", data:{ "id_check" : id_check, "barcode" : barcode },
			success: function(rs)
			{
				var rs = $.trim(rs);
				if(rs == "")

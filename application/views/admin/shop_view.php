...
function add_shop()
{
	...
	$.ajax({
		...
		type:"POST", cache:"false", data:{ "shop_code" : code },
		success: function(rs)
		{
			var rs = $.trim(rs);
			var rs = parseInt(rs);
			...
			else
			{
				...
				$.ajax({
					...
					type: "POST", cache: false, data:{ "shop_code" : code, "shop_name" : name },
					success: function(rd)
					{
						load_out();
						var rd = $.trim(rd);
...
function add_row(shop_code, shop_name)
{
	var source 	= $("#add_template").html();
	var data 		= { "shop_code" : shop_code, "shop_name" : shop_name };
	var ed 		= $("#edit").val();
	var de		= $("#delete").val();
...
function save_edit()
{
	...
	$.ajax({
		...
		type:"POST", cache:"false", data:{ "code" : code, "original_code" : original_code },
		success: function(rs)
		{
			var rs = $.trim(rs);
			if( rs > 0 )
			...
			else
			{
				$.ajax({
					...
					type:"POST", cache: false, data:{ "shop_code" : code, "shop_name" : name, "original_code" : original_code },
					success: function(rs)
					{
						load_out();
						var rs = $.trim(rs);

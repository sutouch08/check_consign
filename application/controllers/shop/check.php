...
public function do_checking()
{
	if( $this->input->post("id_check") && $this->input->post("barcode") )
	{
		...
		if($rd)
		{
			...
			if($rs)
			{
				$re = array("id" => $rs, "barcode" => $barcode, "product" => $rd, "timestamp" => date("H:i:s") );	
				$re = json_encode($re);
			}
			else
			{
...
public function get_history()
{
	...
	if($data)
	{
		foreach($data as $rs)
		{
			$arr = array("id" => $rs->id_check_detail, "barcode" => $rs->barcode, "product" => $this->item_code($rs->barcode), "timestamp" => date("H:i:s", strtotime($rs->date_add)));
			array_push($ds, $arr);
		}
	}
	else
